        ...
        def merge(row):
            ...
            for i in range(len(row)):
                if pair:
                    ...
                    # 此处new_game是棋盘对象，有耦合的问题
                    new_game.score += 2 * row[i]
                    pair = False
                else:
...
class Screen(object):
    ...
    def __init__(
        self, screen=None, grid=None, score=0, best_score=0, over=False, win=False
    ):
        self.grid = grid
        self.score = score
        self.over = over
...
class GameManager(object):
    ...
    def is_over(self):
        self.over = not any(
            self.can_move(move)
            for move in ["up", "down", "left", "right"]  # 解决bug，不应该包括其他状态
        )
    ...
    def _restart_or_exit(self):
        self.screen.draw()
        # return "init" if self.action.get() == Action.RESTART else "exit" # 键盘如果不是R就退出
        return "init" if self.action.get() == Action.RESTART else "game"
    ...
    # 当实例直接调用时，会触发此函数，例如instance=Object()，instance()-->此时触发call
...
if __name__ == "__main__":
    # 注意，此处会生成一个GameManager的实例，然后curses.wrapper自动调用实例，
    # 并将命令行窗口对象传入，因此会依次触发init和call方法
    # curses.wrapper(GameManager(2))
    new_game = GameManager(2)
